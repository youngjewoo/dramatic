import BreakLineModel from '../document/BreakLineModel';
import DocumentModel from '../document/DocumentModel';
import ImageModel from '../document/ImageModel';
import LinkModel from '../document/LinkModel';
import ParagraphModel from '../document/ParagraphModel';
import RunModel from '../document/RunModel';
...
const contentsAry = [
  ...
  new ParagraphModel(
    [
      new RunModel({
        text: '가장 이해가 잘 됐던 글은 아무래도 구글에서 작성한 이 글이 아닐까 생각되네요.',
      }),
    ],
    [urls[0]],
  ),
  new BreakLineModel(),
  new LinkModel('Google CRP 링크', urls[1]),
  new BreakLineModel(),
  new BreakLineModel(),
  new ParagraphModel([
    new RunModel({
  ...
  new LinkModel('모던 브라우저 동작 과정 Part.3', urls[1]),
];
